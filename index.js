...
var timerupdate;
var audio1 = new Audio('./suii.mp3');
var audio2 = new Audio('./bgm.mp3');
var undostatearray = [];
var redostatearray = [];
...
function updateTimer() {
    ...
    timercount.innerHTML = 'Sisa waktu : ' + mins + ':' + secs;
    time--;
    win_condition();
    if (time < 0) {
        alert("Anda kalah, waktu habis!");
        undostate.disabled = true;
        redostate.disabled = true;
        startgame.disabled = true;
        resetgame.disabled = false;
...
function showNoOfPlates() {
    countdiv.innerHTML = 'Jumlah Balok : ' + no_of_plates;
}
...
function showStepCount() {
    ...
    stepcountdiv.innerHTML = 'Total Langkah : ' + step_count;
}
...
function win_condition() {
    ...
    if (bar1.childNodes.length == 0 && bar2.childNodes.length == 0 && bar3.childNodes.length == no_of_plates) {
        ...
        // Tampilkan kanvas dan mulai animasi confetti
        ...
        // Sembunyikan kanvas jika belum menang
        canvas.style.display = 'none';
    }
}
function startConfetti() {
    // Reset the particles for a fresh confetti animation
